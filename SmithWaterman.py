...
#检查输入是否是DNA
def check_dna(seq):
    correct_base=set("ATGC")
    for base in seq :
        if base not in correct_base :
            return False
        else:
            continue
    return True
...
# parser set argument and check ?
def readseqs_from_arg():
    parser = argparse.ArgumentParser(description='Aligning sequences...')
    parser.add_argument('filename1',action="store",help="First sequence file")
    parser.add_argument('filename2',action="store",help="Second sequence file")
...
# 获取文件内的序列
    ...
    with open(file2,'r') as file:
        for line in file :
            if line[0]==">":
                continue
            sequence2.append(line.strip())
        sequence2 = "".join(sequence2)
    return sequence1,sequence2


# build the SW alignment...
def perform_smith_waterman(sequence1,sequence2):
    # values for weights
    global seqmatch
    global seqmismatch
    global seqgap
...
sequence1="AGTGATAAACTAGTAATTTTT"
sequence2="TTGGGGGTAAACAGGGG"
if check_dna(sequence1) and check_dna(sequence2) :
    perform_smith_waterman(sequence1,sequence2)
else:
    print("Input is not correct, please check and try again.")

sequence1,sequence2=readseqs_from_arg()
perform_smith_waterman(sequence1,sequence2)